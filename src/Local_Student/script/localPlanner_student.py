...
class LocalPlanner:
    ...
    def __init__(self, K_linear, K_angular, Sat_linear, Sat_angular, Obstacle_range, Angle_to_allow_linear,
                 Waypoint_error, Destination_error, Angle_error):
        # init params
    ...
    def scanCallback(self, scan):
        ...
        #rospy.loginfo("range min: %f, Obstacle = %s" %(min(scan.ranges), self.isObstacle))
    # ******************************************************************************************
    # **************************************   SERVICES   **************************************
    # ******************************************************************************************
    ...
    def pathService(self, req):
        ...
        try:
            ...
            for i in xrange(len(req.pathToGoal.poses)):
                ...
                req.pathToGoal.poses[i].pose.position.x -= x  # TODO for students : Apply tranform in x
                req.pathToGoal.poses[i].pose.position.y -= y  # TODO for students : Apply tranform in y
                rospy.loginfo("# Pose %d : x = %.2f   y = %.2f" % (
                i, req.pathToGoal.poses[i].pose.position.x, req.pathToGoal.poses[i].pose.position.y))
    ...
    def pathSequencer(self, dist, angle, finalOrientation):
        ...
        if (dist < self.Waypoint_error) and len(self.pathPoses) > 1:
            ...
            # computeVelocity
            rospy.loginfo(
                "# New goal : x=%f ; y=%f" % (self.pathPoses[0].pose.position.x, self.pathPoses[0].pose.position.y))
            ...
            if fabs(finalOrientation) >= self.Angle_error:
                state = "Last Goal position Reached"  # TODO for students : return string matching with the stateReach in progress
            else:
    ...
    def computeVelocity(self, dist, angle, goalState):
        ...
        twist.angular.z = min(angle * self.K_angular, self.Sat_angular)  # TODO for students : Apply gain and saturation (both ROSPARAM) to angle
    ...
    def localPlanning(self):
        ...
        while not rospy.is_shutdown():
            ...
            #rospy.loginfo("dist = %.2f    angle = %.2f State = %s" % (dist, angle, goalState))
...
if __name__ == '__main__':
    try:
        ...
        # Start LocalPlanner#
        # ------------------#
        localPlanner = LocalPlanner(K_LINEAR, K_ANGULAR, SAT_LINEAR, SAT_ANGULAR, OBSTACLE_RANGE, ANGLE_TO_ALLOW_LINEAR,
                                    WAYPOINT_ERROR, DESTINATION_ERROR, ANGLE_ERROR)
